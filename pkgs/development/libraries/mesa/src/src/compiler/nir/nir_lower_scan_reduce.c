...
static nir_def *
mask_for_cluster_size(nir_builder *b, nir_def *mask, unsigned cluster_size)
{
   if (cluster_size < 32) {
                ...
                nir_def *data, unsigned cluster_size)
{
   ...
   mask = mask_for_cluster_size(b, mask, cluster_size);
...
static bool
nir_lower_scan_reduce_intrin(nir_builder *b, nir_intrinsic_instr *intrin,
                             void *opts_)
{
   ...
   nir_lower_scan_reduce_options *opts = opts_;
   if (opts && opts->filter && !opts->filter(&intrin->instr, opts->data))
      return false;
...
bool
nir_lower_scan_reduce(nir_shader *nir, nir_lower_scan_reduce_options *opts)
{
   return nir_shader_intrinsics_pass(nir, nir_lower_scan_reduce_intrin,
                                     nir_metadata_none, opts);
}
