...
static bool
should_lower_scan_reduce(const nir_instr *instr, UNUSED const void *data)
{
   nir_intrinsic_instr *intr = nir_instr_as_intrinsic(instr);

   /* hardware does not support clustered scans/reductions */
   if (nir_intrinsic_has_cluster_size(intr)) {
      unsigned cluster = nir_intrinsic_cluster_size(intr);
      if (cluster && cluster != 32)
         return true;
   }
...
agx_nir_lower_subgroups(nir_shader *s)
{
   ...
   nir_lower_scan_reduce_options sr_opts = {.filter = should_lower_scan_reduce};
   progress |= nir_lower_scan_reduce(s, &sr_opts);
