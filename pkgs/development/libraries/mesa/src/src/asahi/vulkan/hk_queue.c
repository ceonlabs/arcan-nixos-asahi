...
{
   assert(!cs->stream_linked && "todo: cdm stream links");

   *cmd = (struct drm_asahi_cmd_compute){
      .encoder_ptr = cs->addr,
      .encoder_end = cs->addr + (cs->current - cs->start),
             ...
             struct vk_queue_submit *submit)
{
   ...
   for (unsigned i = 0; i < submit->wait_count; ++i) {
      asahi_fill_sync(&waits[i], submit->waits[i].sync,
   ...
   struct drm_asahi_submit submit_ioctl = {
      ...
      .in_sync_count = submit->wait_count,
              ...
              uint32_t index_in_family)
{
   ...
   queue->drm.id = agx_create_command_queue(
      &dev->dev, DRM_ASAHI_QUEUE_CAP_RENDER | DRM_ASAHI_QUEUE_CAP_BLIT |
