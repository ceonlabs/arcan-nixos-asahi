...
static void
hk_destroy_internal_shader(struct hash_entry *ent)
{
   struct agx_shader_part *part = ent->data;
   free(part->binary);

   /* The agx_shader_part itself is ralloc'd against the hash table so will be
    * freed.
    */
}

static void
hk_destroy_internal_shaders(struct hk_internal_shaders *s)
{
   _mesa_hash_table_destroy(s->ht, hk_destroy_internal_shader);
                ...
                const VkAllocationCallbacks *pAllocator, VkDevice *pDevice)
{
   ...
   result = hk_build_write_shader(dev, &dev->write_shader);
   if (result != VK_SUCCESS)
...
fail_write_shader:
   hk_shader_destroy(&dev->vk, &dev->write_shader->vk, NULL);
fail_internal_shaders:
   hk_destroy_internal_shaders(&dev->prolog_epilog);
fail_queries:
