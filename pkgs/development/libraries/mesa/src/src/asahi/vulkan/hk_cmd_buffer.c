                     ...
                     struct vk_command_buffer **cmd_buffer_out)
{
   ...
   result = vk_command_buffer_init(&pool->vk, &cmd->vk, &hk_cmd_buffer_ops, 0);
                    ...
                    struct vk_shader **const shaders)
{
   ...
   for (uint32_t i = 0; i < stage_count; i++) {
      struct hk_shader *shader = container_of(shaders[i], struct hk_shader, vk);
...
{
   enum pipe_shader_type stage = s->b.info.stage;
   ...
   /* TODO: Map uniforms for prologs/epilogs */
