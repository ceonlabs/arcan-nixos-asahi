...
libagx_copy_query(constant struct libagx_copy_query_push *push)
{
   ...
   bool write_results = available || push->partial;

   if (write_results) {
      // For occlusion queries, results[] points to the device global heap,
      // so indices are remapped according to the query pool's allocation.
